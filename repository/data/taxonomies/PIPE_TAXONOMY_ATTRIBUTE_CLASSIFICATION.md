...
Assign one primary downstream classification to every Pipe field currently defined by the 23-column import contract or the 16-field Attribute Dictionary. The classification prevents category/attribute duplication and identifies adapter storage or consumption responsibility before implementation; it never replaces canonical Repository authority.

## Classification Semantics

- Each field has exactly one **Final classification** in this model.
- Final means the single proposed downstream class for Sprint 03C review; it does not mean Founder approval, canonical Product authority, or physical WordPress implementation.
- A field may be consumed by SEO or CRM without becoming SEO Metadata or an Internal CRM Field. The classification identifies primary adapter/storage responsibility only; the use flags identify other downstream consumers. Canonical Product truth follows `Catalog → Platform → Family → Series → Variant Rules → derived SKU`.
...
| ---: | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 1 | `parent_sku` | شناسهٔ کالایی والد | Import Helper Field | Resolves a downstream Parent mapping and future derived Parent SKU/reference; not canonical entity identity, taxonomy, or specification | No | No | No | No | Yes | Yes | Yes |
| 2 | `variation_sku` | شناسهٔ کالایی تنوع | Import Helper Field | Carries a future derived Variation SKU for the downstream adapter; not canonical entity identity, and final syntax/value remains outside this sprint | No | No | No | No | Yes | Yes | Yes |
...
| 26 | `seo_title` | عنوان سئو | SEO Metadata | Canonical public-page/search-intent projection on the Parent page; not Product-fact authority | Yes | No | No | Yes | No | No | Yes |
| 27 | `seo_description` | توضیحات سئو | SEO Metadata | Canonical public-page/search-intent projection on the Parent page; must use approved Repository facts only | Yes | No | No | Yes | No | No | Yes |
| 28 | `canonical_slug` | نامک کانونیکال | SEO Metadata | Canonical Parent public-page URL projection required before publication after URL/SEO approval; not Product identity | Yes | No | No | Yes | Yes | Yes | Yes |
...
- Legacy Product Family `لوله استیل / Stainless Steel Pipe` is the only downstream Pipe category projection in this review model; it does not replace canonical Family/Series identity.
...
- Grade, Finish, Diameter, Thickness, and Length are controlled downstream global-attribute projections of Variant Rules axes.
- Product type, names, Parent/Variation IDs, and derived SKUs are import/adapter helpers, not canonical entity identities, categories, or attributes.
