...
| Canonical Pilot Record | Repository-only approved-or-candidate reference to one exact governed tuple for controlled evaluation | Has its own stable `pilot:` identity; historical `GOLD-PIPE-*` and `PIPE-COMB-*` references are non-identity; it is not a Product, SKU, Variation, availability value, Master Data row, Golden package, import asset, or runtime object |
...
one Approval Evidence. After legal `DRAFT → REVIEW → APPROVED`, all 31 bounded
