...
| OQ-DATA-011 | Which real Product Attribute definitions, Profiles, values, and controlled terms may enter the canonical registries? | `FD-PD02B-001` and [PD-02B Scope](PD02B_MINIMUM_CANONICAL_SLICE_SCOPE_V1.0.md) | Resolved only for the exact DRAFT minimum slice: Material, Grade, `stainless_steel`, `201`, `304`, `316`, and one INTERNAL Family Profile; all other data remains out of scope |
...
| OQ-DATA-013 | Has the PD-02B DRAFT passed independent technical review and received final Founder approval? | Future `PD02B-TECH-REVIEW-001` and `FD-PD02B-001` transition evidence | Open; blocks REVIEW/APPROVED completion and merge |
