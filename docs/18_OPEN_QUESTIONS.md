...
> Project OS 2.0 resolves the operating model and future commerce target. C008-R1 preserves G1 `6/9 / NOT_READY`; C008-FT1 remains a separate false sibling gate with eight unmet launch prerequisites. C009 resolves only one exact internal combination/SKU leaf for the selected 201/51 Pilot. Availability, commercial SKU policy, broader Product scope, gate re-evaluation, Runtime, M4 and successor implementation remain separately gated.
...
Historical Git authorizations retain their original scope and chronology. Semantic state, current authorization, and the next action are maintained only in [Current Project State](CURRENT_PROJECT_STATE.md); this register creates no execution authority. `FD-C000-OS2-001` settles the OS2 operating model, future per-SKU target architecture, PD-03B seed/reference classification, and C1-T03 freeze. C008-R1 preserves G1 `6/9 / NOT_READY`; C008-FT1 remains an independent `FALSE / 4 of 12` gate. C009 promotes only the exact 201/Silver/51/0.50/6m combination and internal SKU leaf from `pilot:f5922666261e`; it does not change C002, Availability or C008-FT1, and grants no commercial, Runtime, M4 or successor authority. Originating authorization references for Wave 2A and Wave 2B remain to be linked; merge history is not a substitute.
...
| OQ-GOV-021 | Should Git Governance be approved, and which independent mirror, backup location, custodians, signing policy, and retention period are authorized? | [Git Governance](GIT_GOVERNANCE.md) and [Baseline Known Limitations](BASELINE_v1.0.md#known-limitations) | Partially resolved: the primary repository, canonical/default `main`, PR #1–#29 integration evidence, main protection, and per-task live-tip resolution are verified. Broader governance approval, mirror, backup, custody, signing, retention, and future merge policy remain open. |
...
| OQ-GOV-023 | Which Product Repository hierarchy is canonical, and how do Parent/Variation commerce constructs relate to it? | `FD-W2G-001`, [Enterprise Product Data Model](19_PRODUCT_DATA_MODEL.md#product-hierarchy), `FD-PD03A-001`, `FD-PD03B-001`, and [C009 Scope](C009_FIRST_COMMERCIAL_SLICE_CANONICAL_LEAF_PROMOTION_SCOPE_V1.0.md) | Partially resolved: canonical hierarchy and downstream Parent/Variation boundary are settled; C009 applies it to exactly one internal combination/SKU leaf. Broader Product/SKU population and downstream commerce projection remain separately gated. |
| OQ-GOV-029 | Which governed evidence and commercially useful boundaries should define a future Fast-Track slice beyond the three PD-03B seed/reference records? | `FD-C000-OS2-001`, `C002-CONTRACTS-001`, [PD-03B Scope](PD03B_CANONICAL_PILOT_SCOPE_V1.0.md), [C009 Scope](C009_FIRST_COMMERCIAL_SLICE_CANONICAL_LEAF_PROMOTION_SCOPE_V1.0.md), and [Implementation Readiness](IMPLEMENTATION_READINESS.md) | Partially resolved for the first internal slice only: C009 binds `pilot:f5922666261e` to one 201/Silver/51/0.50/6m combination and one SKU leaf. Availability, commercial facts, Master/Golden, import, Runtime, launch eligibility and any broader slice remain unselected/separately gated. The other Pilot records remain seeds, not a ceiling. |
...
| OQ-GOV-025 | How is semantic operational state separated from the dynamic Git tip without an infinite post-merge reconciliation loop? | `FD-GOV-XD-00`, [Current Project State](CURRENT_PROJECT_STATE.md), and [Codex Sprint Protocol](CODEX_SPRINT_PROTOCOL.md) | Resolved on 2026-07-28: Current Project State owns semantic state; each task resolves and records the exact live `main` SHA in its Scope/Approval Packet; ordinary tip changes do not trigger reconciliation, while a phase, authorization, gate, or next-action change does. |
| OQ-GOV-026 | Which exact role assignments satisfy executor, domain reviewer, Repository Guardian, QA/Security, rollback owner, and Founder separation of duties? | `FD-GOV-XD-00`, [Implementation Readiness](IMPLEMENTATION_READINESS.md), and future Sprint Approval Packets | Open; role classes are defined, named assignments remain Sprint-specific |
| OQ-GOV-027 | Which explicit exception or safe sequence resolves credential containment under G02 against mandatory pre-mutation backup/restore evidence under G03? | [Execution Gates](EXECUTION_GATES.md) and [Implementation Sequence](IMPLEMENTATION_SEQUENCE.md) | Open; no access, revocation, backup, restore, staging, or runtime action authorized |
| OQ-GOV-028 | Which Review-state Product, Content, SEO, URL, and Schema statements that treat Variable Parent Product as canonical owner must be corrected or classified as non-governing? | `FD-W2G-001`, `FD-W2G-002`, C1-T06 Gate 1 and both Scope Amendments, exact 39-path patch SHA-256 `93f1e6bb1f93faac782a3a17c01166c3e731bcbf4cc8d14c5261f03a60a69434`, two independent zero-finding reviews, merged PR #29, Merge Commit `13531830ad0c4fa57cf8dcab4c5516f27f64e4ea`, and main CI `30796043918` | Resolved on 2026-08-03 for the approved C1-T06 documentation scope; `Catalog → Platform → Family → Series → Variant Rules → SKU` remains canonical and Parent/Variation remains downstream. Product/SKU/fact/value/availability, Knowledge/SEO/WooCommerce implementation, import, runtime, deployment, and production remain separately gated. |
...
| OQ-DATA-002 | Which SKU, stable-ID, stock-state, unit, visibility, supply-after-order, and per-SKU purchase-eligibility evidence policies are approved? | `FD-C000-OS2-001`, [C000 Decision Package](C000_OS2_STRATEGIC_RECONCILIATION_DECISION_PACKAGE.md), [C009 Scope](C009_FIRST_COMMERCIAL_SLICE_CANONICAL_LEAF_PROMOTION_SCOPE_V1.0.md), and [WooCommerce Product Model](20_WOOCOMMERCE_PRODUCT_MODEL.md) | Partially resolved only for one internal stable SKU leaf: C009 assigns one CSPRNG identity under the exact Variant Rule Set without a public commercial SKU code. Availability, stock, price, visibility, purchase eligibility, commercial/legal, Runtime, Production and activation evidence remain open and separately gated. |
...
| OQ-DATA-013 | Has PD-02B received final Founder approval after independent technical PASS? | `PD02B-TECH-REVIEW-001` and `FD-PD02B-001` transition evidence | Resolved: technical attempt 2 PASS and legal `DRAFT → REVIEW → APPROVED` are recorded; PR #23 and main CI `30482348480` later completed Git integration |
| OQ-DATA-014 | Which Founder-confirmed and candidate Brand/size/grade/appearance/length evidence may become exact canonical values and evidence-backed Variant Rules combinations, and how should Product Group presentation map to them? | `FD-C003-DISCOVERY-001`, immutable PD-03A/PD-03B evidence and [C009 Scope](C009_FIRST_COMMERCIAL_SLICE_CANONICAL_LEAF_PROMOTION_SCOPE_V1.0.md) | Partially resolved for exactly one tuple: Stainless Steel / 201 / Silver / 51 / 0.50 / 6m. Brand and Color remain absent; no other value, Pilot, Product Group presentation, Availability or Cartesian combination is promoted. |
| OQ-DATA-015 | Should `SUPPLIER_STATED` become a reviewed C002 Mass Provenance method, and what protected locator, supplier-role, basis, chronology, conflict, reviewer, and non-inference evidence must it require? | C003 discovery registry and [C002 Contract Scope](C002_COMMERCIAL_PILOT_PRODUCT_ADMINISTRATION_CONTRACTS_SCOPE_V1.0.md) | Open architecture proposal requiring separate review; it is not a Founder-accepted candidate, and no mass instance or canonical value is authorized |
...
| OQ-DATA-019 | Which exact 201/51 Brand × Thickness × Appearance × Length tuples are supported by independently reviewed evidence? | `FD-C003-R1-CP03-001`, PD-03B `pilot:f5922666261e`, and [C009 Scope](C009_FIRST_COMMERCIAL_SLICE_CANONICAL_LEAF_PROMOTION_SCOPE_V1.0.md) | Partially resolved only for one brand-neutral tuple: 201 / Silver / 51 mm / 0.50 mm / 6 m, material Stainless Steel. C009 creates exactly one `pcomb:` and one SKU leaf; Brand/Color remain absent and all broader tuple questions remain open. |
| OQ-DATA-020 | What protected gross-profit, demand/repeatability, current supplier, media-rights, SEO and fulfillment-risk evidence resolves the nine C002 criteria? | [C008 Scope](C008_C002_READINESS_REAL_WORLD_EVIDENCE_CLOSURE_SCOPE_V1.0.md), [C008-R1 Scope](C008_R1_C002_REMAINING_REAL_WORLD_EVIDENCE_CLOSURE_SCOPE_V1.0.md), [C008-FT1 Amendment](C008_FT1_FAST_TRACK_INQUIRY_LAUNCH_GOVERNANCE_AMENDMENT_V1.0.md), [C009 Scope](C009_FIRST_COMMERCIAL_SLICE_CANONICAL_LEAF_PROMOTION_SCOPE_V1.0.md), and C002 candidate contract | C009 does not alter this evidence gate. C002 remains `6/9 / NOT_READY`: Supply/Fulfillment are submitted/incomplete, Photo/Content is missing, supplier intake remains deferred—not waived or verified—and rights-safe media remains required. C008-FT1 remains `FALSE / 4 of 12` and is not re-evaluated. |
