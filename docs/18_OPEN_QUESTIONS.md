...
| OQ-DATA-011 | Which real Product Attribute definitions, Profiles, values, and controlled terms may enter the canonical registries? | Future `PD-02B` decision package after [PD-02A Scope](PD02A_CONTROLLED_VALUES_ATTRIBUTE_PROFILES_SCOPE_V1.0.md) | Open; PD-02A creates only synthetic infrastructure and requires `attributes: []`, `value_registries: []`, and `profiles: []` |
| OQ-DATA-012 | Which stable Catalog/Platform/Family scope identity, Grade-versus-Alloy terminology, Color-versus-Finish classification, Product Data owner, and qualified steel-domain reviewer are approved for the minimum canonical Profile? | Future `PD-02B` Founder/domain decision | Open; each item blocks canonical population and PD-03 |
