...
| OQ-DATA-010 | What Contract/Schema/Validator/Test boundary enables Product Data without creating canonical rows? | `FD-PD01-001` and [PD-01 Scope v1.0](PD01_PRODUCT_DATA_CONTRACT_SCOPE_V1.0.md) | Resolved for PD-01 on 2026-07-28: synthetic fixtures only, closed Draft 2020-12 validation, explicit profiles/assignments/combinations, and all readiness flags false; lifecycle remains `DRAFT` until independent review and Founder promotion evidence |
| OQ-DATA-011 | Which real Product Attribute definitions, Profiles, values, and controlled terms may enter the canonical registries? | Future `PD-02` decision package | Open; PD-01 explicitly keeps `repository/data/registries/product-attributes.yaml` empty and creates no canonical data |
