...
Historical Git authorizations retain their original scope. PR #1–#29 are integration evidence, with PR #13 and PR #14 preceding the final PR #12 merge, PR #15 reconciling active governance afterward, PR #16 hardening BP1 M1 accessibility and local validation, PR #17 reconciling the resulting governance state, PR #18 integrating the scoped BP2 lifecycle approval, PR #19 closing that integration cycle, PR #20 integrating `GOV-XD-00`, PR #21 integrating PD-01, PR #22 integrating PD-02A, PR #23 integrating PD-02B, PR #24 integrating PD-03A, PR #25 integrating PD-03B, PR #26 reconciling the post-PD-03B operational state, PR #27 integrating `FD-DS-PROGRAM-001` and completed `C1-T04` with DS-PC=`HOW` and DS-SPD=`WHAT`, PR #28 reconciling the post-PR27 operational state, and PR #29 integrating C1-T06 canonical-owner reconciliation. Semantic state is maintained only in [Current Project State](CURRENT_PROJECT_STATE.md); every task resolves and records its own exact starting GitHub `main` SHA. Consult [Current Project State](CURRENT_PROJECT_STATE.md#approved-next-action) for the sole current next action and mutable authorization and GO/NO-GO boundary; this question register creates no independent authorization and does not restate mutable operational instructions. Product hierarchy and canonical path ownership are settled by `FD-W2G-001` and `FD-W2G-002`; the scoped legacy canonical-owner contradiction is resolved through C1-T06 without creating Product or runtime authority. K-01 is authorized by `FD-K01-001`. `FD-BP2-ADM-001` approves the BP2 Data Administration contract only as documentation governance; implementation authority remains false. Canonical Product values, Product Attribute definitions, WordPress, runtime, content, publication, deployment, production, mirror, backup, signing, and retention questions retain their prior status unless explicitly updated below. Originating authorization references for Wave 2A and Wave 2B remain to be linked; merge history must not be used as a substitute.
...
| OQ-GOV-029 | When may the exact three PD-03B Pilot records advance beyond repository-only status? | [PD-03B Scope](PD03B_CANONICAL_PILOT_SCOPE_V1.0.md) and [Implementation Readiness](IMPLEMENTATION_READINESS.md) | Open beyond PD-03B: Product/SKU, Availability, Master/Golden, Import and Runtime each require later evidence and separate Founder authorization. |
