...
The only canonical Product Repository hierarchy is `Catalog → Platform → Family → Series → Variant Rules → SKU`. Catalog through Variant Rules are canonical Repository concepts. SKU is a derived identifier created only after governed modeling and is not a canonical entity identity. Labels, slugs, WooCommerce IDs, Parent IDs, Variation IDs, and other adapter identifiers likewise do not replace Repository identity.

| Entity | Purpose | Owner | Lifecycle | Relationships | Required fields | Optional fields | Constraints | Future expansion |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Catalog | Canonical root of governed product organization | Product Repository authority | Repository entity lifecycle | Contains Platforms | Stable ID, owner, lifecycle, source | Approved labels and external mappings | Repository source only; no automatic page, URL, taxonomy, or commerce object | Additional catalogs require a separate governing decision |
| Platform | Canonical structural layer within a Catalog | Product Repository authority | Repository entity lifecycle | One Catalog; contains Families | Stable ID, Catalog ID, owner, lifecycle, source | Approved labels and external mappings | Not a website, WordPress instance, or public-page identity by definition | Additional platforms require governed scope and relationship review |
| Family | Canonical product-family concept within a Platform | Product Repository authority | Repository entity lifecycle | One Platform; contains Series | Stable ID, Platform ID, owner, lifecycle, source | Approved labels and downstream mappings | Legacy Product Family labels do not replace this identity | Future mappings/localizations reference its stable ID |
| Series | Canonical product-series concept within a Family | Product Repository authority | Repository entity lifecycle | One Family; governed by Variant Rules | Stable ID, Family ID, owner, lifecycle, source | Approved labels, profiles, and downstream mappings | Legacy Product Type labels do not replace this identity | Expansion requires approved Family relationship and governance |
| Variant Rules | Canonical rules governing permitted variation structure for a Series | Product Repository authority | Repository rule lifecycle | One Series; constrains governed model outputs | Stable ID, Series ID, rule source, owner, lifecycle | Approved attribute/profile references | Candidate value space does not prove a valid combination or availability | Rule evolution requires compatibility and migration review |
| SKU (derived identifier) | Identify a governed model output after required modeling and approval | Product Repository authority pending | Derived-identifier lifecycle | Traces to governed source identities and rules | Derived value, source references, policy/version evidence | Downstream system mappings | Not a canonical entity identity; never derived from a label, slug, row position, Parent ID, or Variation ID | Policy and generation remain separately gated |
| Product projection | Present approved Repository sources in public, content, or commerce contexts | Projection owner pending | Projection lifecycle | Repository source entities, taxonomy, attributes, media, documents, inquiry | Projection ID, stable source references, owner, lifecycle | Public-page/SEO/adapter data | Downstream only; no Product Repository or public-price authority | New projection types require consumer and authority review |
| Product Family (legacy mapping) | Preserve a historical or navigation label mapped to an approved Family source | Navigation/taxonomy projection owner pending | Mapping lifecycle | One approved Family source; optional landing | Mapping ID, source Family ID, label, owner, lifecycle | External mapping and landing | Not a canonical Repository layer or identity; no automatic URL | Future mappings/localizations retain the source Family ID |
| Product Group (legacy mapping) | Provide an optional downstream navigation/presentation grouping | Navigation projection owner pending | Mapping lifecycle | Approved Repository sources and mapped views | Mapping ID, source references, purpose, owner, lifecycle | Landing and external mapping | Not a canonical Repository layer; cannot insert a new hierarchy level | Additional groupings require non-duplication and source-mapping review |
| Product Type (legacy mapping) | Present a downstream navigation, content, or commerce classification | Projection owner pending | Mapping lifecycle | Approved Family/Series sources and profiles | Mapping ID, source references, purpose, owner, lifecycle | SEO/content relationships | Not a canonical Repository layer or WooCommerce `product_type` authority | New mappings require domain/profile/migration review |
| Variable Parent Product (downstream mapping) | Group shared public-page and commerce-adapter presentation | Commerce/page projection owner pending | Adapter/projection lifecycle | Stable Repository sources, governed Variant Rules, downstream Variations, media, documents, knowledge, inquiry | Adapter-scoped ID, stable source references, lifecycle, owner | SEO/page data and external IDs | Does not own Product Repository identity or rules; no public price/cart/checkout | Future channels map back to stable Repository sources |
| Variation (downstream mapping) | Represent one downstream selectable state mapped from a governed tuple | Commerce-adapter owner pending | Adapter/projection lifecycle | One downstream Parent mapping, governed source tuple, inquiry context | Adapter-scoped ID, Parent mapping ID, stable source references, lifecycle | Derived SKU when separately approved, separately approved availability evidence, media/document override, external ID | Not a canonical Repository entity; does not prove availability or gain price authority | Independent public page/URL requires a separate approved exception |
...
| Attribute | Reusable governed specification definition | Attribute owner pending | Attribute governance lifecycle | Series, Variant Rules, values, and downstream product/Variation projections | ID, label, key, definition, value type, owner, lifecycle | Unit, variation/filter/SEO flags | Flat by default; no uncontrolled local duplicates | Attribute-specific hierarchy only by approval |
| Attribute Value | Controlled or validated value of an Attribute | Attribute/domain owner pending | Value lifecycle | Attribute, Variant Rules, downstream projections, synonyms, external mappings | ID/value, attribute ID, source, status | Unit, precision, label, synonyms | Typed/unit-aware; no mixed units or silent synonyms | Localization and external mappings |
...
| Inquiry Line | Product requirement within an Inquiry | Inquiry/Product owner pending | Parent Inquiry lifecycle | Inquiry, stable Repository source references, downstream Parent Product/Variation commerce context, attributes | ID/order, inquiry ID, product context, quantity/unit or approved unknown | Notes, alternatives, stock snapshot | Submission snapshot; no price/order data and no transfer of source authority | External line mapping after approval |
...
| Media Set | Ordered governed media collection for an entity | Product/media owner pending | Parent/product content lifecycle | Repository source entities, downstream Product/Variation projections, Content, Media Assets | ID, owner, applicability, ordered members | Roles, locale | Set does not duplicate source or asset authority | Channel-specific renditions by approval |
| Technical Document | Versioned product/project technical evidence | Qualified technical owner pending | Technical-document lifecycle | Repository source entities, downstream Product/Variation projections, Projects, Media preview | ID, type, title, version, owner, reviewer, access, applicability | Issue date, language, checksum | No unsupported certificate/claim; protected access enforced | External document-system mapping |
...
Schema.org types, search documents, navigation entries, URL records, breadcrumbs, sitemaps, feeds, and external integration payloads are projections of canonical source entities. They require stable source IDs, projection purpose, owner, eligibility, source version, lifecycle, access, canonical public destination when applicable, validation, and refresh behavior. Public URL ownership and external visibility never make them Product Repository identities or business masters.
...
- No relationship is inferred from a common label, tag, URL segment, legacy Product Family label, shared attribute, content co-occurrence, or AI similarity.
...
| 0.1.1 | 2026-08-03 | Reconciled the Product Repository entity hierarchy with `FD-W2G-001` and `FD-W2G-002`; legacy Product/Parent/Variation concepts remain downstream mappings. |
