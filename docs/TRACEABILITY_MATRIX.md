...
| `PD03B-EVIDENCE` | Approval Evidence Contract/Schema/registry/validator | Founder scope/data decision; required `PD03B-TECH-REVIEW-001`; dataset hashes; anti-replay | Reject forged review, hash substitution, premature approval, or replay | DRAFT technical review is pending and nonce is unconsumed |
| `PD03B-TEST` | Closed schemas, offline validators, fixture boundary, 42-case mutation manifest, and `tests/test_pd03b_canonical_pilots.py` | Positive, negative, adversarial, lifecycle, identity, reference, availability, readiness, and schema boundaries | Every counted mutation must dispatch and fail closed | Passing tests cannot promote lifecycle status |
