...
| `PD03A-DATA` | PD-03A prerequisite Contract/Schema/extension registry/validator | Founder Series, naming, axis, Unit, precision, and no-claim decisions | Exact 2 entities, 4 Attributes, 1 Finish Registry/Term, 1 six-rule INTERNAL Series Profile, and 11 labels | DRAFT records are Candidate; PD-02B aggregate registries remain immutable |
...
| `PD03A-EVIDENCE` | Dedicated approval Contract/Schema/registry/validator | Founder authorization; failed-review history; future independent technical PASS | Four SHA-256 bindings, no false human PASS, anti-replay, and lifecycle checks | Approval remains Candidate in DRAFT |
| `PD03A-TEST` | Synthetic pilot Contract/Schema/validator, fixtures, mutation manifest, and `tests/test_pd03a_foundation.py` | Exact three reference tuples and fail-closed Test Contract | 25 mutations dispatched through real loaders/validators; PD-02B regression retained | Synthetic fixtures cannot be promoted or treated as Pilots |
| `PD03A-REVIEW` | `PD03A-TECH-REVIEW-001` | Independent technical review required before REVIEW | `PENDING` in DRAFT | PASS evidence, lifecycle transitions, conditional Git gates, and main CI remain required |
